...
if(isset($_POST["submit"])){
        if(isset($_POST['sid2'])){
            ...
            $selectat = "SELECT MaDM FROM danhmucsach WHERE TenDM = '$categtemp'";
            $resultat = mysqli_query($conn,$selectat);
            while($rowat = mysqli_fetch_assoc($resultat)){
                $categ = $rowat['MaDM'];
            }
            $addsql = "INSERT INTO theloaisach(TenTL, MaDM) VALUES ('$title','$categ')";
        ...
        if(isset($_POST['sid'])){
            ...
            $updatesql = "UPDATE theloaisach SET TenTL='$title', MaDM='$categ' WHERE MaTL='$id'";
            ...
            <div class="crud-user">
                <h3 class="crud-user_heading">Danh Sách Thông Tin Sách</h1>
                    <div class="crud-user_content">
                            <div class="btnAdd">
                            <a class="btnAdd-a" href="addCategory.php?sid2=dir">Thêm thể loại</a>  
                            </div>        
                        <table class="crud-user_list">
                            <tr class="crud-user_titlebar">
                                <th class="crud-user_column-name">Mã thể loại</th>
                                <th class="crud-user_column-name">Tên thể loại</th>
                            ...
                            <?php
                            require_once '../connection.php';
                            $selectsql = "SELECT theloaisach.MaTL, theloaisach.TenTL, danhmucsach.TenDM
                            FROM theloaisach 
                            INNER JOIN danhmucsach ON theloaisach.MaDM = danhmucsach.MaDM";
